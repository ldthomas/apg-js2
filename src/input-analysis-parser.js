module.exports = function() {
	...
	// }
	/**
	 * Read the grammar file into memory.
	 */
	...
	/**
	 * Parse for preliminary checking - Uses resource/grammar.bnf to generate
	 * the parser.
	 */
	...
	var convert = function(filename, end) {
		...
		try {
			...
			that.lines.forEach(function(val, index) {
				count = fs
						.writeSync(fd, buf, val.beginChar, val.textLength);
	...
	/**
	 * Converts all line ends (CRLF, LF, CR or EOF) to CRLF, including last line
	 * returns filename if successful, false if not
	 */
	this.toCRLF = function(filename) {
		convert(filename, CRLF);
	};
	/**
	 * Converts all line ends (CRLF, LF, CR or EOF) to LF, including last line
	 * returns filename if successful, false if not
	 */
	...
	this.toHtml = function(className) {
		...
		this.lines.forEach(function(val, index) {
			html += '<tr>';
			html += '<td>' + val.lineNo + '</td><td>' + val.beginChar
					+ '</td><td>' + val.length + '</td><td>'
					+ apglib.utils.charsToHtml(that.chars, val.beginChar, (val.beginChar + val.length -1));
					+ '</td>';
			html += '</tr>\n';
		});
