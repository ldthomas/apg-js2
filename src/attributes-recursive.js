/*
 * Determine the recursive attributes, left, nested, right & cyclic.
 * Assumes non-recursive attributes, finite & empty have already been determined.
 * Follows the same logic and flow of non-recursive attributes
 * but with different rules for the recursive attribute discovery.
...
module.exports = function(rules) {
	...
	var ruleAttr = function(startIndex, rule, attr) {
		while (true) {
			...
			if(found !== -1){
				// use attributes of competed branch rule
	...
	var isCatNested = function(childAttrs){
		...
		for (var i = 0; i < childAttrs.length; i += 1) {
			...
			if(foundRecursive){
				...
				if(child.right === true && child.left === false && child.notEmpty === true){
					foundRecursive = true;
				}
			}
		}
		...
		for (var i = childAttrs.length-1; i >= 0; i -= 1) {
			...
			if(foundRecursive){
				...
				if(child.left === true && child.right === false && child.notEmpty === true){
					foundRecursive = true;
				}
			}
		}
		...
		for (var i = 0; i < childAttrs.length; i += 1) {
			...
			if(foundLeft === false){
				...
				if(foundRecursive === false){
					if(child.left === true || child.right === true || child.cyclic === true){
						foundRecursive = true;
					}
				}else{
					if(child.notEmpty === true){
	...
	for(var i = 0; i < rules.length; i += 1){
		if (rules[i].ctrl.type === id.ATTR_R || rules[i].ctrl.type === id.ATTR_MR || rules[i].ctrl.type === id.ATTR_RMR) {
			var rule = rules[i];
